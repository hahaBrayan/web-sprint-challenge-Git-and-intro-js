...
function get20s(arr) {
  ...
  for (let i = 0; i<arr.length; i++) {
    //yearBornAndDied is an array: [YYYY, -, YYYY]
    //If yBAD[0] & [2] is within 19th century, push to new array
    const yearBornAndDied = arr[i].years.split(' ');
    if (yearBornAndDied[0] >1899 && yearBornAndDied[2] <2000) {
...
function lotsOfArt(arr) {
  const newArr = [];
  for (let i = 0; i < arr.length; i++) {
    const paintedOver100 = arr[i].paintings > 100;
    if (paintedOver100) {
      newArr.push(arr[i].name);
    }
  }
...
function artistInfo(arr, name) {
  const newArr = [];
  const i = arr.findIndex(artist => artist.name === name)
  newArr.push(arr[i].bio);
  return newArr
}
